...
/// <summary>
/// Tests for 6502 BMI (Branch if Minus) instruction
///
/// BMI branches when the negative flag is set (1):
/// - Branches when negative flag = 1 (result of previous operation was negative)
/// - Does not branch when negative flag = 0
...
{
    [Fact]
    public void BMI_Branches_When_Negative_Flag_Set()
    {
        var instructionInfo = InstructionSet.GetInstruction(0x30);
        var instruction = new DisassembledInstruction
        {
            Info = instructionInfo,
            Bytes = [0x30, 0x05], // Branch forward by 5 bytes
    ...
    [Fact]
    public void BMI_Does_Not_Branch_When_Negative_Flag_Clear()
    {
        var instructionInfo = InstructionSet.GetInstruction(0x30);
        var instruction = new DisassembledInstruction
        {
    ...
    [Fact]
    public void BMI_Backward_Branch_When_Negative_Flag_Set()
    {
        var instructionInfo = InstructionSet.GetInstruction(0x30);
        var instruction = new DisassembledInstruction
        {
            Info = instructionInfo,
            Bytes = [0x30, 0xFB], // Branch backward by -5 (0xFB = -5 in signed byte)
        ...
        var allInstructions = new List<Ir6502.Instruction>
        {
            ...
            // Check if we should continue (set negative flag if X < 3, clear negative flag when X >= 3 to exit loop)
            new Ir6502.Binary(
                Ir6502.BinaryOperator.LessThan,
                new Ir6502.Register(Ir6502.RegisterName.XIndex),
                new Ir6502.Constant(3),
                new Ir6502.Flag(Ir6502.FlagName.Negative)), // Set negative flag when X < 3 (loop should continue)

            // Add the BMI instruction (will branch if negative flag is set)
            nesIrInstructions[0]
        };
        ...
        jit.TestHal.GetFlag(CpuStatusFlags.Negative).ShouldBeFalse(); // Final state: negative flag clear (loop exit condition)
    }

    [Fact]
    public void BMI_Does_Not_Affect_Other_Flags()
    {
        var instructionInfo = InstructionSet.GetInstruction(0x30);
        var instruction = new DisassembledInstruction
        {
        ...
        var allInstructions = new List<Ir6502.Instruction>
        {
            ...
            new Ir6502.Copy(new Ir6502.Constant(1), new Ir6502.Flag(Ir6502.FlagName.Negative)), // Set negative for branch
    ...
    [Fact]
    public void BMI_Does_Not_Affect_Registers()
    {
        var instructionInfo = InstructionSet.GetInstruction(0x30);
        var instruction = new DisassembledInstruction
        {
        ...
        var allInstructions = new List<Ir6502.Instruction>
        {
            // Set negative flag set for branch to occur
            new Ir6502.Copy(new Ir6502.Constant(1), new Ir6502.Flag(Ir6502.FlagName.Negative)),
    ...
    [Fact]
    public void BMI_Forward_Branch_Maximum_Positive_Offset()
    {
        var instructionInfo = InstructionSet.GetInstruction(0x30);
        var instruction = new DisassembledInstruction
        {
            Info = instructionInfo,
            Bytes = [0x30, 0x7F], // Maximum forward branch (+127)
    ...
    [Fact]
    public void BMI_Backward_Branch_Maximum_Negative_Offset()
    {
        var instructionInfo = InstructionSet.GetInstruction(0x30);
        var instruction = new DisassembledInstruction
        {
            Info = instructionInfo,
            Bytes = [0x30, 0x80], // Maximum backward branch (-128)
        ...
        var allInstructions = new List<Ir6502.Instruction>
        {
            ...
            new Ir6502.Binary(
                ...
                new Ir6502.Flag(Ir6502.FlagName.Negative)), // Set negative flag if X == 0
    ...
    [Fact]
    public void BMI_With_Various_Negative_States_From_Previous_Operations()
    {
        var instructionInfo = InstructionSet.GetInstruction(0x30);
        var instruction = new DisassembledInstruction
        {
        ...
        // Test case 1: After operation that clears negative flag (positive/zero result)
        {
            var allInstructions = new List<Ir6502.Instruction>
            {
            ...
            jit1.TestHal.SetFlag(CpuStatusFlags.Negative, false);
        ...
        // Test case 2: After operation that sets negative flag (negative result)
        {
            ...
            jit2.TestHal.SetFlag(CpuStatusFlags.Negative, true);
